...
public class ProxyWorker : IWorker
{
        ...
        IProxyConsumer[] proxyConsumers)
    {
        ...
        _worker = workerController.CreateWorker(() => RevalidateProxy().GetAwaiter().GetResult(),
            _proxyRevalidateInterval);
    }
    ...
    private async Task RevalidateProxy()
    {
        ...
        _logger.Trace(
            $"{GetType().Name}: Проверяем закешированные прокси-сервера. Количество серверов: {_cachedProxies.Count}");
    ...
    private void SetNewProxy(WebProxy newProxy, WebProxy oldProxy)
    {
        ...
        _logger.Info(
            $"{GetType().Name}: Был изменён адрес прокси сервера. {oldProxy?.Address?.ToString() ?? "<NONE>"} ==> {newProxy?.Address?.ToString() ?? "<NONE>"}");
