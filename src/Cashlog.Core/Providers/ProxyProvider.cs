...
public class ProxyProvider : IProxyProvider
{
    ...
    private static readonly string _proxyListUrl =
        @"https://proxy-list.org/russian/search.php?search=ssl-yes&country=any&type=any&port=any&ssl=yes";
    ...
    {
        var response = await _proxyListUrl.WithHeaders(new { Accept = "text/plain", User_Agent = _userAgent })
            .GetAsync();
        ...
        {
            _logger.Warning(
                $"Количество элементов в {nameof(httpNodes)} и {nameof(proxyNodes)} было разное. Невозможно получить новый прокси адрес.");
        ...
        for (var i = 0; i < proxyNodes.Count; i++)
        {
            var match = Regex.Match(proxyNodes[i].InnerHtml, $"Proxy\\('(?<addressBase>{base64Regex})'\\)");
            if (!match.Success)
                continue;
