...
internal static class Urls
{
    ...
    internal static string GetCheckUrl(string fiscalNumber, string fiscalDocument, string fiscalSign, DateTime date,
        decimal sum)
    {
        ...
        return
            $"https://proverkacheka.nalog.ru:9999/v1/ofds/*/inns/*/fss/{fiscalNumber}/operations/1/tickets/{fiscalDocument}" +
            $"?fiscalSign={fiscalSign}&date={date.ToString(DateFormat)}&sum={GetSum(sum)}";
    }

    private static string GetSum(decimal sum)
    {
        return $"{Convert.ToInt32(Math.Round(sum, 2, MidpointRounding.AwayFromZero) * 100)}";
    }
    ...
    internal static string GetReceiveUrl(string fiscalNumber, string fiscalDocument, string fiscalSign)
    {
        ...
        return
            $"https://proverkacheka.nalog.ru:9999/v1/inns/*/kkts/*/fss/{fiscalNumber}/tickets/{fiscalDocument}?fiscalSign={fiscalSign}&sendToEmail=no";
    }
}