...
internal sealed class MessagesMainHandler : IMessagesMainHandler
{
        ...
        switch (userMessageInfo.Message.QueryData.MenuType)
        {
            ...
            case MenuType.NewReceiptAdd:
            {
                ...
                if (receipt.Status == ReceiptStatus.New || receipt.Status == ReceiptStatus.NewManual)
                {
                    ...
                    receipt.PurchaseTime = DateTime.UtcNow;
    ...
    {
        // TODO Вынести проверку на верхний уровень.
        if (!_messageHandlersMap.TryGetValue(MessageType.QrCode, out var handlers))
        {
            throw new InvalidOperationException("Message handlers for QR code not found");
        }

        if (handlers.Length > 1)
        {
            throw new InvalidOperationException($"Expected `1` QR code message handler, but got: `{handlers.Length}`");
        }
